...
namespace ConnectEduV2.Pages.Class
{
    public class ClassDetailModel : PageModel
    {
        ...
        private readonly Constant constant = new Constant();
            ...
            else
            {
                ...
                if (acc.RoleId == 2)
                {
                    ...
                    if (check == null)
                    {
                        ...
                        registration.Date = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, TimeZoneInfo.FindSystemTimeZoneById("SE Asia Standard Time"));
                    ...
                    if (check != null && check.RegistrationStatusId == 1002)
                    {
                        ClassRegistration registration = new ClassRegistration();
                        registration.UserId = acc.Id;
                        registration.ClassId = classID;
            ...
            else
            {
                ...
                }else if (acc.RoleId == 3)
                {
                    ...
                    ClassChat chat = new ClassChat
                    {
                        ...
                        ChatDate = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, TimeZoneInfo.FindSystemTimeZoneById("SE Asia Standard Time")) // Sử dụng thời gian Việt Nam
                    };
        ...
        //khi kết thúc lớp học thì cập nhật tất cả đơn đăng ký lớp học thành đã hoàn thành chờ student xác nhận
        public IActionResult OnGetChangeStatus(int classid, string action)
        {
            var aClass = _classRepository.GetSingleById(classid);
            if (aClass != null && action.Equals("End"))
            {
                aClass.ClassStatusId = constant.End;
                _classRepository.Update(aClass);
                _classRepository.SaveChanges();

                var regis = _classRegistrationRepository.GetMulti(x => x.ClassId == classid);
                foreach (var reg in regis)
                {
                    if(reg.RegistrationStatusId == constant.Studying)
                    {
                        reg.RegistrationStatusId = constant.Complete;
                        _classRegistrationRepository.Update(reg);
                        _classRegistrationRepository.SaveChanges();
                    }
                }
            }
            ...
            {
                aClass.ClassStatusId = constant.Start;
                _classRepository.Update(aClass);
                _classRepository.SaveChanges();
                var regis = _classRegistrationRepository.GetMulti(x => x.ClassId == classid);
                foreach (var reg in regis)
                {
                    if (reg.PaymentStatusId == constant.Paid)
                    {
                    reg.RegistrationStatusId = constant.Studying;
                    _classRegistrationRepository.Update(reg);
                    _classRegistrationRepository.SaveChanges();
